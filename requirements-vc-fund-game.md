...
- **投資額の15%を即時回収**（死亡時は0%回収）※[変更26]
- ポートフォリオから除去してアクションを1消費
- 確認ダイアログに「投資額 / 部分回収(15%) / 実質損失」の内訳を表示
...
**ポートフォリオサイドバーのソート順** ※[変更27]
- 生存中（growing/stable/struggling）: ファンドリターン貢献度（現在価値÷取得価額の倍率）降順
- 清算済み（dead/exited_*）: 末尾にまとめて表示
...
### [変更28] 成長判定結果にファンド名バッジを表示 — commit `9092277`

各銘柄の成長判定カードに、保有ファンドを示すバッジを追加。

- リード投資家: インジゴ色バッジ + ★マーク
- フォロー投資家: グレーバッジ
- 未保有銘柄: バッジなし

**影響ファイル**: `src/screens/phases/GrowthPhase.tsx`

---

### [変更27] ポートフォリオサイドバーのソート順変更 — commit `bc4a9c2`

サイドバーのポートフォリオ一覧に明示的なソート順を適用。

- 生存中の銘柄: ファンドリターン貢献度（倍率 = 現在価値÷取得価額）の降順
- 清算済み（dead/exited_*）: 末尾にまとめて表示
...
### [変更26] 損切り時の部分回収（15%）を実装 — commit `9fccc91`

損切りの戦略的価値を高めるため、`WRITE_OFF` 実行時に投資額の15%を即時回収する仕組みを追加。

- 損切り実行 → 投資額 × `WRITE_OFF_RECOVERY_RATE`（0.15）を `remainingCapital` と `realizedReturns` に加算
- 死亡時は 0% 回収のまま → 早期損切りに明確なメリット（15%回収 vs 0%）
- 確認ダイアログに「投資額 / 部分回収(15%) / 実質損失」の内訳を表示
- カード本体にも「回収可能: +X万円」を常時表示

**影響ファイル**: `src/data/constants.ts`, `src/context/GameContext.tsx`, `src/screens/phases/DealIndividualPhase.tsx`
